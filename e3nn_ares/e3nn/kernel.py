...
class Kernel(torch.nn.Module):
    ...
    def forward(self, r, r_eps=0, custom_backward=False):
        ...
        *size, xyz = r.size()
        ...
        sel = radii == 0
        filter_order = self.set_of_l_filters[0]
    ...
    for i, (mul_out, l_out, p_out) in enumerate(Rs_out):
        ...
        for j, (mul_in, l_in, p_in) in enumerate(Rs_in):
            ...
            for k, l_filter in enumerate(l_filters):
                ...
                # Add CG negative correction.
                C = correct_cg(C, l_in, l_filter, l_out)
...
def correct_cg(x, i, f, o):
    # CG coeffs correction for ARES. 

    # Other sqrt factor is due to CG coeffs vs. wigner 3-j symbols conventions.
    # See https://en.wikipedia.org/wiki/Clebsch%E2%80%93Gordan_coefficients#Relation_to_Wigner_3-j_symbols
    if (i, f, o) == (0, 1, 1):
        x = -x
    if (i, f, o) == (1, 0, 1):
        x = -x
